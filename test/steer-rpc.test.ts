...
describe("subagent steering handle RPC transport", () => {
  test("handle.steer sends a prompt command with streamingBehavior steer (not a raw steer command)", async () => {
    ...
    assert.ok(Array.isArray(assistant.content), "expected assistant content blocks");
    const text: unknown = assistant.content[0];
    assert.ok(text !== null && typeof text === "object" && "type" in text && text.type === "text");
    assert.ok("text" in text && typeof text.text === "string", "expected echoed RPC commands as text");
    const lines: unknown = JSON.parse(text.text);
    assert.ok(Array.isArray(lines), "expected an array of RPC lines");
    const received = lines.map((line: unknown) => {
      assert.ok(typeof line === "string");
      const command: unknown = JSON.parse(line);
      assert.ok(command !== null && typeof command === "object" && !Array.isArray(command));
      return command;
    });

    // No raw "steer" RPC commands at all — they bypass the input hook.
    assert.ok(
      !received.some((cmd) => "type" in cmd && cmd.type === "steer"),
      `expected no raw steer commands, got: ${JSON.stringify(received)}`,
    );

    const steered = received.find((cmd) => "message" in cmd && cmd.message === "steer me");
    assert.ok(steered, `expected the steering message to arrive, got: ${JSON.stringify(received)}`);
    assert.ok("type" in steered);
    assert.equal(steered.type, "prompt");
    assert.ok("streamingBehavior" in steered);
    assert.equal(steered.streamingBehavior, "steer");

    // The initial task prompt is still delivered as a plain prompt.
    const initial = received.find(
      (cmd) => "message" in cmd && typeof cmd.message === "string" && cmd.message.includes("test task"),
    );
    assert.ok(initial, "expected the initial task prompt");
    assert.ok("type" in initial);
