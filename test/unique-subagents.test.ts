...
function worker(overrides: Partial<SingleResult> = {}): SingleResult {
  return {
    ...
    messages: [],
    stderr: "",
    usage: { ...emptyUsage(), cost: 2, turns: 1 },
    toolCalls: {},
    completedTurns: 1,
...
function context<T extends { type: string; id?: string }>(entries: T[]) {
...
test("resuming one named agent adds usage but not another agent", () => {
  const entries = [
    entry("launch", worker()),
    entry("resume", worker({ task: "follow up", usage: { ...emptyUsage(), cost: 3 } }), "resume_subagents"),
  ];
  const line = collectCombinedUsageStatusLine(context(entries))!;
  assert.match(line, /WITH SUBS: \(1\) Σ \$5\.0000/);
  const fork = entry(
    "fork",
    worker({ name: "ADA", sessionDir: "/different/fork", usage: { ...emptyUsage(), cost: 1 } }),
    "resume_subagents",
  );
...
test("nested resumes preserve unique identities through compact serialization", () => {
  ...
  const nestedResume = entry(
    "nested-resume",
    worker({ name: "Bo", usage: { ...emptyUsage(), cost: 3 } }),
    "resume_subagents",
  );
  const parent = worker({
    usage: { ...emptyUsage(), cost: 1 },
    messages: [nestedLaunch.message, nestedResume.message],
  });
  const durable: unknown = JSON.parse(JSON.stringify(details(parent)));
  assert.ok(isSubagentDetails(durable));
  assert.ok(durable.usageSummary);
  ...
  const later = entry(
    ...
    worker({
      usage: { ...emptyUsage(), cost: 1 },
      messages: [
        entry("child-again", worker({ name: "Bo", usage: { ...emptyUsage(), cost: 1 } }), "resume_subagents").message,
      ],
    }),
    "resume_subagents",
  );
...
test("unnamed agents use stable budget or session identities, not their task text", () => {
  ...
  assert.match(
    collectCombinedUsageStatusLine(
      context([entry("launch", first), entry("resume", resumedFork, "resume_subagents")]),
    )!,
    /\(1\) Σ \$4\.0000/,
  );
  ...
  assert.match(
    collectCombinedUsageStatusLine(
      context([entry("one", second), entry("two", third), entry("one-again", second, "resume_subagents")]),
    )!,
    /\(2\) Σ \$6\.0000/,
  );
});
...
test("the name registry repairs legacy nested counts and refreshes without a new chat entry", () => {
  ...
  try {
    ...
    const agents: Record<string, { sessionDir?: string }> = { Ada: { sessionDir: "/sessions/ada" }, Bo: {}, Cy: {} };
    ...
    const identity = {
      ...
      data: { namesFile: file, ownerId: "root" },
    };
