...
const BASE_SUBAGENTS_TOOL_DESCRIPTION =
  "Run agents in separate processes. Pass { tasks: [{ agent, task, max_agents_allowed }] }. Calls block until all tasks finish; put parallel tasks in one array.";
...
const SUBAGENT_USAGE_GUIDANCE = [
  ...
  "2. Before launching, say what each agent will do and why delegation helps. Set max_agents_allowed on every task: include the assigned agent and everyone below it. For example 1 for a direct worker, or 2 for a worker that launches its own reviewer. The number you choose is exactly the number of slots reserved. Siblings get separate shares; unused shares stay reserved for resumes. Count only workers the planned work needs, not speculative teams. The whole call is rejected if the total exceeds your remaining slots. Respect any tighter user limit.",
