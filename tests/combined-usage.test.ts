...
test("combined usage counts every billed parent entry, like Pi's own footer", () => {
  const entries = [
    ...
    { id: "a2", branch: "abandoned", type: "message", message: { role: "assistant", provider: "anthropic", usage: usage(200, 20, 2) } },
...
test("picker search matches by name fragment, agent type, and task", () => {
  ...
  assert.equal(filterPickerItems(items, "caro").map((item) => item.name).join(), "Carolyn");
  assert.deepEqual(
    filterPickerItems(items, "reviwer").map((item) => item.name).sort(),
    ["Jayavarman", "Natalie"],
  );
